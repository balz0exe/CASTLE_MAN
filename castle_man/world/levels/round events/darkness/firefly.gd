...
const MAX_Y            := -100.0  # ← adjust to your world's ceiling
var _target     : Vector2
...
func _process(delta: float) -> void:
	...
	_recycle_if_offscreen()
func _recycle_if_offscreen() -> void:
	var cam := get_viewport().get_camera_2d()
	var center := cam.get_screen_center_position()
	var half := get_viewport().get_visible_rect().size / 2.0 / cam.zoom
	var margin := half * 1.5
	var bounds := Rect2(center - margin, margin * 2)
...
func _pick_new_target() -> void:
	...
	_target.y = maxf(_target.y, MAX_Y)  # ← clamp so fireflies never go above ceiling
